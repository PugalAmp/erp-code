    ...
    public function get_total_hours_in_month($user_id, $payment_month)
{
    ...
    $attendance_info = $this->payroll_model
        ->get_attendance_info_by_date($start_date, $end_date, $user_id);
    ...
    if (!empty($attendance_info)) {
        foreach ($attendance_info as $row) {
            ...
            if (
                empty($row->date_in) ||
                empty($row->clockin_time) ||
                empty($row->date_out) ||
                empty($row->clockout_time)
            ) {
                continue;
            }

            // Convert to timestamps
            $startdatetime = strtotime($row->date_in . ' ' . $row->clockin_time);
            $enddatetime   = strtotime($row->date_out . ' ' . $row->clockout_time);
            ...
            // ✅ Skip if checkout is before checkin
            if ($enddatetime <= $startdatetime) {
                continue;
            }

            // Calculate difference in minutes
            $difference_in_minutes = floor(($enddatetime - $startdatetime) / 60);

            $total_minutes += $difference_in_minutes;
        }
    }
